#ifndef CRYPTOGRAPHY_SHA512_H
#define CRYPTOGRAPHY_SHA512_H
...
namespace Xale::Cryptography
{
    ...
    class SHA512
    {
        private:
            ...
            static constexpr std::size_t _outputSize = 64;
            // SHA-512 internal block size in bytes.
            static constexpr std::size_t blockSize = 128;
            // SHA-512 state size (number of 64-bit words).
            static constexpr std::size_t _stateSize = 8;
            // SHA-512 number of rounds.
            static constexpr std::size_t _rounds = 80;
	        // Round constants K (first 80 primes, cube-root fractional parts)
            static constexpr std::array<std::uint64_t, _rounds> _K = {
        ...
        private:
            // Core computing
            static std::array<std::uint64_t, _stateSize> process(
                   const std::array<std::uint64_t, _stateSize> state, 
                   const std::uint8_t block[blockSize]); 
            
            // Helpers
            static std::array<std::uint64_t, _stateSize> initialState();
            static std::array<std::uint8_t, _outputSize> stateToDigest(const std::array<std::uint64_t, _stateSize>& state);
    };
}

#endif // CRYPTOGRAPHY_SHA512_H